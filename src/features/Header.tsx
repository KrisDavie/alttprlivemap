...
import { setHistoryLenToShow, setZoomLevel, toggleFollowPlayer, toggleSomariaPits } from "./map/mapSlice"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
function Header(props: any) {
  ...
  const historyLengthToShow = useAppSelector(state => state.maps.historyLenToShow)
  ...
  return (
    <div className="flex flex-col h-12 items-center">
      <div className="flex absolute top-4 left-2 space-x-2 justify-between">
          ...
          <Checkbox onClick={handleSomariaPits} checked={somariaPits} id="somaria"/>
          <Label htmlFor="somaria" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Show Somaria Pits</Label>
          </div>
          <Separator orientation="vertical" className="flex h-4" />
          <div className="flex items-center space-x-2">
          <Checkbox onClick={handleFollowPlayer} checked={followPlayer} id="follow" />
          <Label htmlFor="follow" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Follow Player</Label>
          </div>
          <Separator orientation="vertical" className="flex h-4" />
          <div className="flex items-center space-x-2 w-48">
          <Slider onValueChange={handleZoomChange} value={[zoomLevel]} defaultValue={[1]} max={5} step={0.1} className={cn("w-[85%]")} id="follow"/>
          <Label htmlFor="follow" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Zoom</Label>
          </div>
          <Separator orientation="vertical" className="flex h-4" />
          <div className="flex items-center space-x-2 w-48">
          <Slider onValueChange={handleHistoryLengthChange} defaultValue={[10]} max={100} step={1} className={cn("w-[60%]")} id="history"/>
          <Label htmlFor="history" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Steps ({historyLengthToShow})</Label>
          </div>
